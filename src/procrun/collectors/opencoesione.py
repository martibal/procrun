...
EXPECTED_HEADERS: Final[tuple[str, ...]] = (
    "CodiceProgramma_ProgrammeID", "Programma_Programme",
    "CodiceLocaleProgetto_OperationLocalIdentifier", "CodiceUnicoProgetto_UniqueProjectCode",
    "TitoloProgetto_OperationName", "SintesiProgetto_OperationSummary",
    "DataAggiornamento_LastUpdate", "CodiceFiscaleBeneficiario_BeneficiaryTaxCode",
    "NomeBeneficiario_BeneficiaryName", "CostoTotale_TotalCost",
    "CostoAmmesso_TotalEligibleExpenditure", "Fondo_Fund", "Ciclo_Period",
    "CategoriaOperazione_CategoryIntervention", "ObiettivoSpecifico_SpecificObjective",
    "DataInizioOperazione_OperationStartDate", "DataFineOperazione_OperationEndDate",
    "TassoCofinanziamentoUE_EUCofinancingRate", "Paese_Country", "CodicePostale_Postcode",
)
SOURCE_ONLY_HEADERS: Final[frozenset[str]] = frozenset({
    "CodiceFiscaleBeneficiario_BeneficiaryTaxCode", "NomeBeneficiario_BeneficiaryName",
})
...
def _extract_single_csv(payload: bytes) -> bytes:
    try:
        with zipfile.ZipFile(io.BytesIO(payload)) as archive:
            ...
            if len(members) != 1:
                raise OpenCoesioneSchemaError(f"expected exactly one CSV in approved ZIP, found {len(members)}")
...
def parse_operation_list_zip(payload: bytes, *, source_url: str) -> OpenCoesioneBatch:
    ...
    for row_number, row in enumerate(reader, start=2):
        ...
        if not operation_id:
            raise OpenCoesioneRowError(f"row {row_number} lacks operation id; whole batch rejected")
        if not operation_name and not operation_summary:
            raise OpenCoesioneRowError(f"row {row_number} lacks both operation name and summary; whole batch rejected")
        if not operation_name:
            operation_name = operation_summary
        if not operation_summary:
            operation_summary = operation_name
        updated_on = _parse_date(row["DataAggiornamento_LastUpdate"], field="list_updated_on", required=True)
        assert updated_on is not None
        updates.add(updated_on)
        staged.append(OpenCoesioneOperation(
            operation_id=operation_id,
            cup=row["CodiceUnicoProgetto_UniqueProjectCode"].strip() or None,
            operation_name=operation_name,
            operation_summary=operation_summary,
            start_date=_parse_date(row["DataInizioOperazione_OperationStartDate"], field="start_date"),
            end_date=_parse_date(row["DataFineOperazione_OperationEndDate"], field="end_date"),
            total_cost_eur=_parse_decimal(row["CostoTotale_TotalCost"], field="total_cost"),
            eligible_expenditure_eur=_parse_decimal(row["CostoAmmesso_TotalEligibleExpenditure"], field="eligible_expenditure"),
            eu_cofinancing_rate=_parse_decimal(row["TassoCofinanziamentoUE_EUCofinancingRate"], field="eu_cofinancing_rate"),
            ...
            intervention_category=row["CategoriaOperazione_CategoryIntervention"].strip() or None,
    ...
    return OpenCoesioneBatch(
        operations=tuple(staged), observed_at=datetime.now(timezone.utc), source_url=source_url,
        source_sha256=hashlib.sha256(payload).hexdigest(), list_updated_on=next(iter(updates)),
    )
...
def to_funding_projects(batch: OpenCoesioneBatch) -> tuple[FundingProject, ...]:
    ...
    for operation in batch.operations:
        projects.append(FundingProject(
            ...
            approved_funding_eur=_conservative_whole_euros(operation.eligible_expenditure_eur),
            ...
            region="Lombardia", municipality=None, nuts_code="ITC4", source_url=operation.source_url,
        ))
