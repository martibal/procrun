    ...
    for i in range(12):
        rows.append(
            StratificationRecord(
                ...
                scope_length_band=ScopeLengthBand.SHORT if i % 2 == 0 else ScopeLengthBand.LONG,
                component_count_band=ComponentCountBand.ONE if i % 3 == 0 else ComponentCountBand.MULTI,
                procurement_band=(
                    ProcurementBand.KNOWN_PROCUREMENT,
                    ProcurementBand.NO_RELEVANT_TED_FOUND,
                    ProcurementBand.AMBIGUOUS_CANDIDATE,
                )[i % 3],
                expected_project_state=ProjectState.UNRESOLVED if i == 4 else ProjectState.CLOSED,
                ...
                description_precision_band=(
                    DescriptionPrecisionBand.HIGH if i % 2 == 0 else DescriptionPrecisionBand.LOW
                ),
...
def test_stratified_selection_is_deterministic_and_covers_required_strata() -> None:
    ...
    first = select_stratified_benchmark(projects, records, selection_seed="round-001", target_size=10)
    second = select_stratified_benchmark(tuple(reversed(projects)), tuple(reversed(records)), selection_seed="round-001", target_size=10)

    assert [p.operation_code for p in first.projects] == [p.operation_code for p in second.projects]
    assert len(first.projects) == 10


def test_missing_domain_fails_closed() -> None:
    records = tuple(
        record.model_copy(update={"domains": tuple(d for d in record.domains if d != "ports_coastal")})
        for record in _records()
    )
    projects = tuple(_project(record.operation_code, i) for i, record in enumerate(records))

    with pytest.raises(StratificationError, match="missing supported domains"):
        select_stratified_benchmark(projects, records, selection_seed="round-001", target_size=10)
