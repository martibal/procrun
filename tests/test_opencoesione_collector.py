...
def _valid_row() -> list[str]:
    return [
        "PRL", "PR FESR Lombardia 2021-2027", "OP-1", "CUP1", "Water upgrade",
        "New pumps and controls", "2026-08-31", "LEGAL123", "Comune X", "1000000",
        "800000", "FESR", "2021-2027", "Water", "OBJ", "2026-01-01", "2027-12-31",
        "0.6", "IT", "00100",
    ]


def _zip_csv(headers: tuple[str, ...] = EXPECTED_HEADERS, rows: list[list[str]] | None = None) -> bytes:
...
def test_known_valid_row_is_accepted() -> None:
    operation = parse_operation_list_zip(_zip_csv(), source_url="https://example.test/source.zip").operations[0]
...
def test_missing_summary_uses_exact_title_as_source_evidenced_fallback() -> None:
    row = _valid_row(); row[5] = ""
    operation = parse_operation_list_zip(_zip_csv(rows=[row]), source_url="https://example.test/source.zip").operations[0]
    assert operation.operation_name == "Water upgrade"
...
def test_canonical_mapping_uses_cup_for_ted_linkage_and_never_retains_identity() -> None:
    project = to_funding_projects(parse_operation_list_zip(_zip_csv(), source_url="https://example.test/source.zip"))[0]
...
def test_fractional_eligible_expenditure_maps_down_without_overstatement() -> None:
    row = _valid_row()
    row[10] = "42030.52"
    batch = parse_operation_list_zip(_zip_csv(rows=[row]), source_url="https://example.test/source.zip")
...
def test_canonical_mapping_falls_back_to_local_id_when_cup_is_absent() -> None:
    row = _valid_row(); row[3] = ""
    project = to_funding_projects(parse_operation_list_zip(_zip_csv(rows=[row]), source_url="https://example.test/source.zip"))[0]
...
def test_unknown_zip_redirect_still_fails_closed() -> None:
    with pytest.raises(OpenCoesioneSchemaError, match="outside frozen route"):
        _validate_zip_final_url("https://opencoesione.gov.it/media/open_data/beneficiari/2021-2027/other.zip")
