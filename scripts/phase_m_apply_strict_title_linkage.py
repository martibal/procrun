...
RATIONALE = "Strict project-title linkage plus frozen structural procurement evidence covers the component."
STOPWORDS = {
    "a","ad","ai","al","alla","alle","allo","con","da","dal","dalla","delle","dei","del","di","e","ed","il","in","la","le","lo","nel","nella","per","su","un","una",
    "lavori","opera","opere","progetto","intervento","realizzazione","riqualificazione","manutenzione","adeguamento","fornitura","servizio","servizi","sistemazione","messa","sicurezza",
}
...
def tokens(value: str | None) -> set[str]:
    if not value:
        return set()
    return {t.casefold() for t in TOKEN_RE.findall(value) if t.casefold() not in STOPWORDS and not t.isdigit()}


def strict_title_match(project_title: str | None, title: str | None, scope: str | None) -> tuple[bool, int, int]:
...
def main() -> int:
    ...
    with psycopg.connect(args.database_url) as conn:
        rows = conn.execute("""
            WITH latest AS (
              SELECT DISTINCT ON (component_id) version_id,assessment_id,component_id,operation_code,state,cutoff_date,
        ...
        with conn.transaction():
            for row in rows:
                (current_version_id,assessment_id,component_id,operation_code,state,cutoff_date,
                 matching_candidates,accepted_evidence_ids,rationale,coverage_note)=row
                assessment = ComponentAssessment(component_id=component_id,state=ComponentState(state),cutoff_date=cutoff_date,
                    rationale=rationale,evidence_ids=tuple(accepted_evidence_ids or ()),coverage_note=coverage_note)
                ...
                if state == "OPEN":
                    ...
                    for ev_version, ev_id, ev_title, ev_scope in evidences:
                        ...
                        structural = all(bool(f.get(k)) for k in (
                            "geography_match","high_scope_overlap","cpv_or_category_match","compatible_date_window"
                        ))
                        if not structural:
                            continue
                        ...
                        if ok:
                            ...
                            audit["reason"] = f"strict title linkage matched {matched}/{total} significant project-title tokens"
                            qualified.append((UUID(str(ev_version)), str(ev_id), audit))
                    if qualified:
                        qualified.sort(key=lambda x: x[1])
                        ev_version, ev_id, audit = qualified[0]
                        assessment = ComponentAssessment(component_id=component_id,state=ComponentState.CLOSED,
                            cutoff_date=cutoff_date,rationale=RATIONALE,evidence_ids=(ev_id,),coverage_note=coverage_note)
                        corrected = list(matching_candidates or []) + [audit]
                        write = append_assessment_version(conn,assessment_id=assessment_id,operation_code=operation_code,
                            assessment=assessment,as_of=now,rule_version=RULE_VERSION,model_version=None,
                            matching_candidates=corrected,accepted_evidence_version_ids=(ev_version,),rejected_evidence=())
            ...
            for operation_code, items in by_operation.items():
                ...
                append_project_assessment_version(conn,assessment=pa,
                    component_assessment_version_ids=tuple(v for v,_ in items),as_of=now,
