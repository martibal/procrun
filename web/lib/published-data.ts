...
function text(value: unknown, name: string): string {
  if (typeof value !== "string" || !value) throw new Error(`${name} must be non-empty text`);
  return value;
}
...
function span(value: unknown, source: string, sourceBaseOffset = 0): SourceSpan {
  ...
  if (!Number.isInteger(start) || !Number.isInteger(end) || start < sourceBaseOffset || end <= start) {
    throw new Error("invalid source span offsets");
  }
  const localStart = start - sourceBaseOffset;
  const localEnd = end - sourceBaseOffset;
  if (localStart < 0 || localEnd > source.length || source.slice(localStart, localEnd) !== valueText) {
    throw new Error("source span is not verbatim");
  }
  return { source_field: text(row.source_field, "source field"), text: valueText, start, end };
}
...
export function parseRunwayProject(value: unknown): RunwayProject {
  ...
  if (!Number.isInteger(sourceStart) || !Number.isInteger(sourceEnd) || sourceStart < 0 || sourceEnd <= sourceStart || sourceEnd - sourceStart !== sourceText.length) {
  ...
  const components = componentsRaw.map((item) => {
    ...
    const projectEvidence = span(component.project_evidence, sourceText, sourceStart);
    const matchesRaw = Array.isArray(component.procurement_matches) ? component.procurement_matches : null;
    ...
    const matches = matchesRaw.map((matchValue) => {
      ...
      if (!Number.isInteger(evidenceStart) || !Number.isInteger(evidenceEnd) || evidenceStart < 0 || evidenceEnd <= evidenceStart) {
      ...
      return {
        ...
        cpv_codes: Array.isArray(match.cpv_codes) ? match.cpv_codes.map(String) : [],
        estimated_value_eur: match.estimated_value_eur == null ? null : Number(match.estimated_value_eur),
        nuts_code: match.nuts_code == null ? null : String(match.nuts_code),
        project_reference: match.project_reference == null ? null : String(match.project_reference),
        evidence: {
    ...
    if (!["OPEN", "CLOSED", "UNRESOLVED"].includes(state)) throw new Error("invalid component state");
    return {
  ...
  if (!["OPEN", "CLOSED", "PARTIAL", "UNRESOLVED"].includes(state)) throw new Error("invalid project state");
  const unresolvedRaw = Array.isArray(row.unresolved_source_evidence) ? row.unresolved_source_evidence : [];
  const unresolved = unresolvedRaw.map((item) => span(item, sourceText, sourceStart));
  return {
    ...
    approved_funding_eur: row.approved_funding_eur == null ? null : Number(row.approved_funding_eur),
    ...
    source_evidence: {
      source_type: text(sourceEvidence.source_type, "source type") as "Project title" | "Project description",
    ...
    orchestration_version: text(row.orchestration_version, "orchestration version"),
    component_rule_version: text(row.component_rule_version, "component rule version"),
    match_rule_version: text(row.match_rule_version, "match rule version"),
    project_classifier_version: text(row.project_classifier_version, "classifier version"),
...
export async function loadPublishedProjects(): Promise<RunwayProject[]> {
  ...
  if (!path) throw new Error("PROCRUN_PUBLISHED_JSONL_PATH is not configured");
...
export function toOpportunities(projects: readonly RunwayProject[]): Opportunity[] {
  return projects.flatMap((project) => {
    const base = {
      ...
      unresolvedEvidence: project.unresolved_source_evidence.map((item) => item.text),
      valueEur: project.approved_funding_eur ?? undefined,
      geography: [project.region, project.nuts_code].filter(Boolean).join(" · ") || "Not stated",
    ...
    return project.components.map((component) => ({
      ...
      openWording: component.state === "OPEN" ? component.state_explanation : undefined,
      procurementEvidence: component.procurement_matches[0]?.evidence.text,
      cpvCodes: [...new Set(component.procurement_matches.flatMap((match) => match.cpv_codes))],
    }));
  });
}
...
export function toCsv(items: readonly Opportunity[]): string {
  const header = [
    "id", "project_title", "component", "state", "cutoff_date", "coverage",
    "coverage_wording", "project_evidence_type", "project_evidence", "unresolved_source_evidence",
  ...
  return [header.join(","), ...items.map((item) => [
    item.id, item.projectTitle, item.component, item.state, item.cutoffDate, item.coverage,
    item.openWording ?? "", item.projectEvidenceType, item.projectEvidence,
    item.unresolvedEvidence.join(" | "), item.geography, item.sourceVersion,
  ].map(quote).join(","))].join("\n");
}
