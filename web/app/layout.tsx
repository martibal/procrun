...
export const metadata: Metadata = {
  title: { default: "ProcRun — Infrastructure procurement intelligence", template: "%s | ProcRun" },
  description: "Evidence-first infrastructure procurement intelligence for suppliers in Lombardia.",
};

export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  const clerkFrontendConfigured = Boolean(process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY?.trim());
  const body = clerkFrontendConfigured ? <ClerkProvider>{children}</ClerkProvider> : children;
  return (
