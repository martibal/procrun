...
export default async function ComponentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <section className="card" style={{marginTop:16}}>
      <p className="small">Assessment</p>
      <p><strong>{item.state}</strong></p>
      <p className="small">{item.state === "OPEN" ? item.openWording : item.procurementEvidence ?? "Insufficient evidence for a safe OPEN/CLOSED conclusion."}</p>
