...
import { loadCustomerView, type CustomerOpportunity, type TedMatchStatus } from "@/lib/customer-view";
...
export default async function RunwayPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const tedStatus = params.ted ?? "ALL";
  ...
  const filtered = view.opportunities.filter((item) => {
    ...
    if (tedStatus !== "ALL" && item.tedStatus !== tedStatus) return false;
    ...
    return [
      ...
      item.tedStatus,
      ...item.procurementMatches.flatMap((match) => [match.notice_id, match.publication_date]),
    ]
      .filter(Boolean)
      .join(" ")
      .toLocaleLowerCase()
  ...
  const tedOptions: TedMatchStatus[] = [
  ...
  return <>
    <h1 className="h1">Projects and procurement status</h1>
    <p className="lede">Browse funded projects and see the published source text, observable TED facts and ProcRun's separate interpretation.</p>
    ...
    <section className="section runway-section">
      ...
      <p className="small">Use the filters to narrow the list by ProcRun interpretation, observable TED status, purchasing need, location or funding. Source text, TED facts and ProcRun interpretation stay separate.</p>

      <details className="status-help">
        <summary>What do the ProcRun interpretations mean?</summary>
        <div className="status-guide" aria-label="Interpretation definitions">
      ...
      <form className="project-filters" method="get" action="/app">
        <label className="filter-search"><span>Search</span><input type="search" name="q" defaultValue={params.q ?? ""} placeholder="Project, programme, notice ID, location or need" /></label>
        <label><span>ProcRun interpretation</span><select name="state" defaultValue={state}><option value="ALL">All interpretations</option><option value="OPEN">Open</option><option value="CLOSED">Closed</option><option value="UNRESOLVED">Unresolved</option></select></label>
        <label><span>TED status</span><select name="ted" defaultValue={tedStatus}><option value="ALL">All TED statuses</option>{tedOptions.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
